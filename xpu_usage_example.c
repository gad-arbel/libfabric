...
/*
 * xpu_usage_example.c - application-side usage of the OFI XPU API
 *
 * This shows everything an application must do to drive XPU-initiated
 ...
 *
 * The device-side kernel that consumes these handles cannot live in a .c
 * file - it is compiled by the XPU toolchain. See the KERNEL SIDE block at
 * the bottom of this file for the companion .cu source and its build line.
 *
 ...
 *
 * Build (host side, without CUDA - callbacks compile to stubs, ops = NULL):
 *   cc -o xpu_usage_example xpu_usage_example.c -lfabric
 *
 * Build (host side, with the CUDA-backed callbacks):
 *   cc -DXPU_EXAMPLE_HAVE_CUDA -o xpu_usage_example xpu_usage_example.c \
 *      -lfabric -lcuda
 */
...
#define XPU_EXAMPLE_HAVE_CUDA

#ifdef XPU_EXAMPLE_HAVE_CUDA
#include <cuda.h>
#endif
...
#define XPU_DEVICE	0		/* device ordinal */
 ...
 *   - free() receives the address alloc() returned in *addr, not the base of
 *     any over-allocation used to satisfy the alignment request.
 */

#ifdef XPU_EXAMPLE_HAVE_CUDA

/*
 * alloc() must be able to free by the address it handed out, but honoring
 * an alignment larger than cuMemAlloc()'s guarantee requires over-allocating
 * and returning an interior pointer. Keep the base around so free() works.
 */
...
static int cuda_check(CUresult res, const char *what)
{
...
{
	CUdeviceptr base, aligned;
	size_t alloc_size = size;
	int slot, sync_flag = 1;
	CUresult res;
	...
	res = cuMemAlloc(&base, alloc_size);
	if (cuda_check(res, "cuMemAlloc"))
		return -FI_ENOMEM;

	aligned = alignment > 1 ?
		  (base + alignment - 1) & ~((CUdeviceptr) alignment - 1) : base;

	/*
	 * Required whenever the NIC DMAs into this memory concurrently with
	 * kernel access - without it CUDA may reorder around the DMA.
	 */
	res = cuPointerSetAttribute(&sync_flag, CU_POINTER_ATTRIBUTE_SYNC_MEMOPS,
				    aligned);
	if (cuda_check(res, "cuPointerSetAttribute(SYNC_MEMOPS)"))
		goto err;

	allocs[slot].base = base;
	allocs[slot].aligned = aligned;
	allocs[slot].dmabuf_fd = -1;

	if (flags & FI_XPU_ALLOC_DMABUF) {
		/*
		 * The fd must cover a page-aligned range, so export from the
		 * page start and report where *addr lands inside it. This
		 * mirrors cuda_get_dmabuf_fd() in src/hmem_cuda.c.
		 */
		size_t page = (size_t) sysconf(_SC_PAGESIZE);
		CUdeviceptr page_base = aligned & ~((CUdeviceptr) page - 1);
		size_t range = ((aligned + size - 1) | (page - 1)) + 1 -
			       page_base;

		res = cuMemGetHandleForAddressRange((void *) fd, page_base,
					range, CU_MEM_RANGE_HANDLE_TYPE_DMA_BUF_FD,
					0);
		if (cuda_check(res, "cuMemGetHandleForAddressRange"))
			goto err;

		*offset = (uint64_t) (aligned - page_base);
		allocs[slot].dmabuf_fd = *fd;
	} else {
		*fd = -1;
		*offset = 0;
	}
			      ...
			      uint64_t size, uint64_t flags, void **dev_addr)
{
	...
	res = cuMemHostRegister(host_addr, size, reg_flags);
	if (cuda_check(res, "cuMemHostRegister"))
		return -FI_EIO;

	res = cuMemHostGetDevicePointer(&dptr, host_addr, 0);
	if (cuda_check(res, "cuMemHostGetDevicePointer")) {
		cuMemHostUnregister(host_addr);
		return -FI_EIO;
	}
...
static void example_xpu_free(uint64_t device, void *addr)
{
	...
	for (slot = 0; slot < MAX_ALLOCS; slot++) {
		if (allocs[slot].aligned != (CUdeviceptr) addr)
			continue;
		if (allocs[slot].dmabuf_fd >= 0)
			close(allocs[slot].dmabuf_fd);
		cuMemFree(allocs[slot].base);
...
static int copy_to_device(void **dev, const void *host, size_t size)
{
	...
	if (cuda_check(cuMemAlloc(&dptr, size), "cuMemAlloc"))
		return -FI_ENOMEM;
	if (cuda_check(cuMemcpyHtoD(dptr, host, size), "cuMemcpyHtoD"))
		return -FI_EIO;
...
int main(void)
{
	...
	/*
	 * iface + device identify the target XPU. ops is optional; when
	 * supplied, ops->size must be set so the provider can tell which
...
#ifdef XPU_EXAMPLE_HAVE_CUDA
	{
		int fd;
		uint64_t off;
		CHECK(example_xpu_alloc(XPU_DEVICE, BUF_SIZE, 4096, 0,
					&buf, &fd, &off));
	}
#else
	...
	mr_attr.device.cuda = XPU_DEVICE;
	...
	/*
	 * Each handle is an opaque blob beginning with struct fi_xpu_hdr. Its
	 * size is provider-defined and returned here; the provider owns the
	 * memory, so keep the objects open for as long as the kernel runs.
	 */
	CHECK(fi_ep_export_xpu(ep, 0, &xpu_ep, &ep_size));
	CHECK(fi_cq_export_xpu(cq, 0, &xpu_cq, &cq_size));
	CHECK(fi_cntr_export_xpu(cntr, 0, &xpu_cntr, &cntr_size));
	...
	CHECK(fi_mr_get_desc(mr, raw_desc, &len, FI_XPU, xpu_ctx));

	/* --- 7. stage everything in device memory and run the kernel ------ */

	CHECK(copy_to_device(&d_ep, xpu_ep, ep_size));
	CHECK(copy_to_device(&d_cq, xpu_cq, cq_size));
	CHECK(copy_to_device(&d_cntr, xpu_cntr, cntr_size));
	CHECK(copy_to_device(&d_addr, raw_addr, ctx_attr.av_addr_size));
	CHECK(copy_to_device(&d_desc, raw_desc, ctx_attr.mr_desc_size));

	/*
	 * launch_xpu_send(d_ep, d_cntr, d_addr, d_desc, buf, BUF_SIZE);
	 *
	 * Defined in the companion .cu file below. The host does not
...
/*
 * ===========================================================================
 * KERNEL SIDE - companion xpu_usage_kernel.cu
 * ===========================================================================
 *
 * Build:
 *   nvcc -c xpu_usage_kernel.cu -I/path/to/libfabric/include
 *
 * Note that nothing here links against libfabric: fi_xpu_device.h is a
 * header-only inline API that dispatches on the provider id embedded in each
 * exported handle. Every one of these calls returns -FI_ENOSYS until a
 * provider adds its case to the dispatch switches in fi_xpu_device.h.
 *
 *   #include <rdma/fi_xpu_device.h>
 *
 *   extern "C" __global__ void
 *   xpu_send_kernel(void *ep, void *cntr, void *peer, void *desc,
 *                   void *buf, size_t len)
 *   {
 *           // Sample the counter before posting so the wait below is not
 *           // satisfied by an unrelated earlier completion.
 *           uint64_t prev = fi_xpu_cntr_read(cntr, FI_XPU_WORK_ITEM);
 *
 *           // desc and peer are the raw blobs from fi_mr_get_desc() and
 *           // fi_av_lookup2(), not a struct fid_mr * or an fi_addr_t.
 *           int ret = fi_xpu_send(ep, buf, len, desc, 0, peer, NULL, 0,
 *                                 FI_XPU_WORK_ITEM);
 *           if (ret)
 *                   return;
 *
 ...
 *   // doorbell for the whole group.
 *   extern "C" __global__ void
 *   xpu_write_kernel(void *ep, void *peer, void *desc, void *buf, size_t len,
 *                    uint64_t raddr, uint64_t rkey)
 *   {
 *           fi_xpu_write(ep, buf, len, desc, 0, peer, raddr, rkey, NULL, 0,
 *                        FI_XPU_WORK_GROUP);
 *   }
 *
 *   extern "C" void
 *   launch_xpu_send(void *ep, void *cntr, void *peer, void *desc,
 *                   void *buf, size_t len)
 *   {
 *           xpu_send_kernel<<<1, 1>>>(ep, cntr, peer, desc, buf, len);
